...
void TRM::enterTemperaturePauses(){
    ...
    for(byte i = 0; i < 6; i++){
        ...
        while(!startStopButton.Clicked()){ //ввод температуры
            if(downButton->Clicked()){temp1--;}            
            if(upButton->Clicked()){temp1++;}
            temp1 = constrain(temp1, 0, 255);
        }
        while(!startStopButton.Clicked()){ //ввод времени
            if(downButton->Clicked()){time1--;}
            if(upButton->Clicked()){time1++;}
        }
...
void TRM::settings(){ //настройки 1-паузы, 2-вкл?выкл wifi, 3-параметры по мощности, 4-диапазон времени(сек/мин), 5- отложенное время
    if(settingsButton.Clicked()){
        ...
        while(1){
            ...
            if(upButton->Clicked()){i++; i = constrain(i, 0, 4);}// Перемещение кнопками up down
            if(downButton->Clicked()){i--; i = constrain(i, 0, 4);}
            if(startStopButton.Clicked()){
                ...
                case 0: //Настройки паузы (ok)
                    ...
                    while(1){//сначала выбираем номер паузы, затем вводим паузу
                        ...
                        if(numberButton->Clicked()){numberPause++; if(numberPause==3){numberPause = 0;}}
                        if(startStopButton.Clicked()){enterTemperaturePauses();}
                    }
                    delete numberButton;
                    break;
                ...
                case 2: // параметры по мощности (ok)
                    ...
                    while(1){
                        if(settingsButton.Clicked()){break;}
                        if(upButton->Clicked()){j++; j = constrain(j, 0, 1);}
                        if(downButton->Clicked()){j--; j = constrain(j, 0, 1);}
                        if(startStopButton.Clicked()){
                            while(1){
                                if(settingsButton.Clicked()){break;}
                                if(upButton->Clicked() && j == 0){powerMax++; powerMax = constrain(powerMax, 0, 100);}
                                if(upButton->Clicked() && j == 1){powerMin++; powerMax = constrain(powerMax, 0, 100);}
                                if(downButton->Clicked() && j == 0){powerMax--; powerMax = constrain(powerMax, 0, 100);}
                                if(downButton->Clicked() && j == 1){powerMin--; powerMax = constrain(powerMax, 0, 100);}
                            }
